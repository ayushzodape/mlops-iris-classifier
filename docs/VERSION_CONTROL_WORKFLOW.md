...
**Rule:** No one commits directly to `main`. All changes flow: `feature/*` → Pull Request → `develop` → (periodically) merged into `main`.
...
Large or generated files are excluded from Git and are expected to be tracked separately (e.g., via DVC, cloud storage, or Git-LFS) rather than committed directly:
...
[Add any project-specific notes here — e.g., recurring conflict areas, naming conventions adopted, tools used (gh CLI vs web UI), etc.]
